...
Page({
  ...
  ,onLoad: function () {
    ...
    wx.getBackgroundAudioManager().onStop(function(e){ that.playNext();   });
    wx.getBackgroundAudioManager().onPause(function(e){
       that.clearRotate();  
       app.setLastPlayInfo({  status : 'pause' });
    });
    // wx.onBackgroundAudioPlay(function(e){ 
    wx.getBackgroundAudioManager().onPlay(function(e){ 
      console.log(e)
  ...
  ,playNext:function(){
       ...
       //如果网络错误导致死循环？
       wx.request({
              url    : 'https://api.uomg.com/api/rand.music',//'http://www.zhixingai.ren/HMusic/bdServlet',// http://118.178.85.4/HMusic/bdServlet http://127.0.0.1:8080/HMusic/bdServlet
              data   : { method:'play',  'type'  : _type, 'sort': '热歌榜', 'format': 'json' },
              success: function(res) {
                if(res.data && res.data.code == 1){
                    var music = res.data.data;

                    that.setData({ songHash : music.hash , singername: music.artistsname, songname  : music.name ,singerpic : music.picurl || '../../res/img/logo_music.png' });
                    that.setData({ singerPicRotate : 0 });
                    that.setData({
                        curMusicInfo : {
                          dataUrl: music.url,
                          title  : music.name,
                          singer : music.artistsname,
